...
この機体で実測した差（2026-08-14 に測り直し。**表に書いた文をそのまま
両モデルへ通した値**なので、同じ文を入れれば再現する）:

| | 英語 22MB | 多言語 114MB |
|---|---|---|
| 「バグを直す」vs「ハクを直す」（低いほど良い） | **1.0000** | 0.9429 |
| 「設定ファイルを読み込む」vs「コンフィグを読む」（高いほど良い） | 0.9290 | 0.5131 |
| 「設定を読む」vs「DB へ接続する」（低いほど良い） | 0.7736 | 0.2862 |
| 「設定ファイルを読み込む」vs「load the configuration file」（高いほど良い） | **-0.1756** | 0.5751 |
| 「今日はいい天気ですね」vs「設定を読む」（低いほど良い） | 0.7809 | 0.3740 |

**前の版の 3 行目と 5 行目は、書かれた文とは別の文で測った値だった。**
両モデルで測り直したところ、その 2 行だけが記録と食い違い、残り 3 行は
小数第 4 位まで一致した（多言語モデルを作り直したときに気づいた
——モデルが原因なら英語モデルは全行一致するはずで、そうならなかった）。
ここから導いていた「同義と別物の差は 0.018」「多言語で 6 倍」も、
その 2 行に依存していたので下の値へ改める。

英語モデルは日本語だけの組を 0.77〜1.00 に潰していて、**ほとんど
区別していない**（高い値は意味ではなく文字の重なりによる）。同義
（0.9290）と別物（0.7736）の差は 0.155。多言語モデルでは 0.5131 と
0.2862 で 0.227 に広がる。

**差が開くことより、潰れ方が消えることの方が効く。** 英語モデルは
「バグ」と「ハク」を **1.0000**——完全に同一と見る。濁点が語彙に
無いので区別しようがない。日本語と英語の訳文に至っては **-0.1756**
と、無関係より低い値を返す。多言語モデルではそれぞれ 0.9429 と
0.5751 になる。
